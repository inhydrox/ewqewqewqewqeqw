...
exports.run = async (client, message, args) => {
  ...
  let channel = message.mentions.channels.first() 
   ...
   await db.set(`welcome_${message.guild.id}`, channel.id) 
    message.channel.send(new Discord.MessageEmbed().setFooter(`Requested By: ${message.author.tag}`, message.author.avatarURL({dynamic: true})).setColor(`#00BFFF`).setDescription(`Welcome Channel is setup as ${channel}`)) 
  }
